...
import { TilesetPrimitive } from '../src/mvt/render/tileset-primitive';
import { createTileKey } from '../src/mvt/tile/tile-key';
...
describe('tileset-primitive', () => {
  it('loads raw tile data and advances to ready through upload budget', async () => {
    ...
    const loadTile = vi.fn<TileLoader['loadTile']>().mockResolvedValue({
      arrayBuffer: tileBuffer,
      byteLength: tileBuffer.byteLength,
      url: 'https://example.com/4/5/6.pbf',
    });
    const primitive = new TilesetPrimitive({
  ...
  it('uses the current visible tile set to keep rendering in-sync with Cesium globe imagery', async () => {
    ...
    const loadTile = vi.fn<TileLoader['loadTile']>().mockImplementation(async (coordinate) => {
      return {
        arrayBuffer: tileBuffer,
        byteLength: tileBuffer.byteLength,
    ...
    const firstTile = tileStore.getTile(createTileKey({ x: 5, y: 6, z: 4 }));
    ...
    const secondTile = tileStore.getTile(createTileKey({ x: 7, y: 8, z: 4 }));
  ...
  it('renders a ready parent tile as fallback for a recently requested child tile', async () => {
    ...
    let resolveChildLoad!: (value: Awaited<ReturnType<TileLoader['loadTile']>>) => void;
    const childLoadPromise = new Promise<Awaited<ReturnType<TileLoader['loadTile']>>>((resolve) => {
  ...
  it('cancels stale inflight loads and starts newer requested tiles', async () => {
    ...
    const loadTile = vi.fn<TileLoader['loadTile']>().mockImplementation(async (coordinate, signal) => {
