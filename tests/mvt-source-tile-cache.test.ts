...
import { MvtSourceTileCache } from '../src/mvt/tile/mvt-source-tile-cache';

describe('mvt-source-tile-cache', () => {
  it('dedupes inflight loads and reuses cached source tiles', async () => {
    const cache = new MvtSourceTileCache({
      maxByteLength: 1024,
      protectedFrames: 0,
    });
    cache.beginFrame(1);

    let resolveLoad!: (value: Awaited<ReturnType<MvtTileLoader['loadTile']>>) => void;
    const loadPromise = new Promise<Awaited<ReturnType<MvtTileLoader['loadTile']>>>((resolve) => {
      resolveLoad = resolve;
    });

    const loadTile = vi.fn<MvtTileLoader['loadTile']>().mockReturnValue(loadPromise);
  ...
  it('evicts unpinned cached source tiles when over budget', async () => {
    ...
    const loadTile = vi.fn<MvtTileLoader['loadTile']>()
