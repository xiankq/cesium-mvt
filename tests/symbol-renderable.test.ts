...
import { SymbolCollisionIndex } from '../src/mvt/render/symbol-collision';
...
describe('symbol-renderable', () => {
  ...
  it('renders sprite-backed icon symbols from openfreemap bright style', async () => {
    ...
    const parsedTile = parseVectorTile(tileBuffer, styleSet, 16);
    expect(parsedTile.buckets.some(bucket => bucket.type === 'symbol')).toBe(true);
  ...
  it('collapses overlapping duplicate point labels within the same tile', async () => {
    ...
    const duplicateRenderBundle = createTileRenderBundle({
      coordinate,
      parsedTileData: parseVectorTile(duplicateFeatureTileBuffer, styleSet, coordinate.z),
      styleSet,
      tilingScheme: new WebMercatorTilingScheme(),
    });

    duplicateRenderBundle.update({}, coordinate.z, new SymbolCollisionIndex());
  ...
  it('renders stacked multilingual point labels as centered multiline text', async () => {
    ...
    const renderBundle = createTileRenderBundle({
      coordinate: { x: 13423, y: 6452, z: 14 },
      parsedTileData: parseVectorTile(createTestMvtArrayBuffer({
        water_name: {
          features: [
            {
              geometry: {
    ...
    renderBundle.update({}, 14, new SymbolCollisionIndex());
    renderBundle.update({}, 14, new SymbolCollisionIndex());
  ...
  it('shares collision hiding across neighboring tiles in the same frame', async () => {
    ...
    const sharedCollisionIndex = new SymbolCollisionIndex();
    const leftBundle = createTileRenderBundle({
      coordinate: { x: 100, y: 100, z: 8 },
      parsedTileData: parseVectorTile(createTestLocalMvtArrayBuffer({
        water_name: {
          features: [{
            geometry: {
    ...
    const rightBundle = createTileRenderBundle({
