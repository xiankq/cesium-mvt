...
import {
  ...
  CesiumMvtPrimitiveLayer,
  CesiumMvtSourceCache,
...
let mvtSourceCache: CesiumMvtSourceCache | undefined
let mvtPreviewLayer: CesiumMvtPrimitiveLayer | undefined
...
function detachMvtRuntime() {
  ...
  mvtPreviewLayer?.destroy()
  mvtPreviewLayer = undefined

  mvtSourceCache?.destroy()
  mvtSourceCache = undefined
...
function attachMvtRuntime(config: OpenFreeMapMvtRuntimeConfig) {
  ...
  viewer.imageryLayers.addImageryProvider(mvtRuntime.provider)
  mvtSourceCache = new CesiumMvtSourceCache({
    scene: viewer.scene,
    scheduler: mvtRuntime.scheduler,
    tilingScheme: mvtRuntime.provider.tilingScheme,
    source: config.source,
  })
  mvtPreviewLayer = new CesiumMvtPrimitiveLayer(
    viewer.scene,
    mvtRuntime.scheduler,
    mvtRuntime.provider.tilingScheme,
    {
      style: config.style,
      sourceCache: mvtSourceCache,
    },
  )
