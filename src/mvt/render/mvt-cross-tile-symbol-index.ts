...
interface SymbolEntry {
  crossTileID: number;
  tileCoordinate: MvtTileCoordinate;
  tileX: number;
  tileY: number;
}
...
export class MvtCrossTileSymbolIndex {
  ...
  removeTile(tileCoordinate: MvtTileCoordinate, layerId: string): void {
    const layerIndex = this.layerIndexes.get(layerId);
    if (!layerIndex)
      return;
  ...
  private getTileKey(coord: MvtTileCoordinate): string {
  ...
  private getTolerance(coord1: MvtTileCoordinate, coord2: MvtTileCoordinate): number {
  ...
  private getScaledCoordinates(
    tileX: number,
    tileY: number,
    sourceCoord: MvtTileCoordinate,
    targetCoord: MvtTileCoordinate,
  ): { x: number; y: number } {
