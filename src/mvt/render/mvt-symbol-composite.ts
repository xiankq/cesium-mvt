...
export interface MvtCompositeSpriteAtlas {
  entries: Map<string, MvtCompositeSpriteEntry>;
  image: HTMLCanvasElement;
  imageUrl: string;
}

export type MvtCompositeSpriteCache = WeakMap<MvtResolvedSymbolLabel, Map<string, MvtCompositeSpriteEntry>>;

export interface MvtCompositeSpriteItem {
  entry: MvtCompositeSpriteEntry;
  icon: MvtResolvedSymbolIcon;
  key: string;
  label: MvtResolvedSymbolLabel;
}

export function createMvtCompositeSpriteCache(): MvtCompositeSpriteCache {
  return new WeakMap<MvtResolvedSymbolLabel, Map<string, MvtCompositeSpriteEntry>>();
}
...
export function getOrCreateCompositeSpriteEntry(
  icon: MvtResolvedSymbolIcon,
  label: MvtResolvedSymbolLabel,
  cache: MvtCompositeSpriteCache,
  fit: string,
  padding: [number, number, number, number],
): MvtCompositeSpriteEntry | undefined {
  ...
  const entry: MvtCompositeSpriteEntry = {
    height,
    width,
    x: 0,
    y: 0,
...
export function buildCompositeSpriteAtlas(
  items: readonly MvtCompositeSpriteItem[],
  atlas: MvtStyleSpriteAtlas,
): MvtCompositeSpriteAtlas {
  ...
  const entries = new Map<string, MvtCompositeSpriteEntry>();
  let currentX = 0;
  let currentY = 0;
  let rowHeight = 0;
