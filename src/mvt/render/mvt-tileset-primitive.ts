...
import type { MvtTileLoader } from '../tile/mvt-tile-loader';
import type { MvtTileStore } from '../tile/mvt-tile-store';
import type { MvtTileRenderBundle } from './mvt-tile-render-bundle';
import { WebMercatorTilingScheme } from '@cesium/engine';
import { isMvtDebugLoggingEnabled, logMvtError } from '../mvt-log';
import { createMvtWarningContext } from '../mvt-warning-context';
import { parseMvtVectorTile } from '../parse/mvt-vector-tile-parser';
import { createMvtFeatureIndex } from '../pick/mvt-feature-index';
import { getMvtSourceTileCoordinate } from '../tile/mvt-overzoom';
import { MvtSourceTileCache } from '../tile/mvt-source-tile-cache';
import { DefaultMvtTileLoader } from '../tile/mvt-tile-loader';
import { MvtParseWorkerClient } from '../worker/mvt-parse-worker-client';
import { MvtSymbolCollisionIndex } from './mvt-symbol-collision';
import { createMvtTileRenderBundle } from './mvt-tile-render-bundle';

export interface MvtTilesetPrimitiveOptions {
  debugLogging?: boolean;
  fallbackFrameWindow?: number;
  getVisibleTileCoordinates?: () => readonly MvtTileCoordinate[];
  ...
  tileLoaderFactory?: (styleSet: MvtStyleSet) => MvtTileLoader;
  tileStore: MvtTileStore;
  tilingScheme?: TilingScheme;
}

export class MvtTilesetPrimitive {
  ...
  private readonly getVisibleTileCoordinates?: () => readonly MvtTileCoordinate[];
  ...
  private readonly renderBundles = new Map<string, MvtTileRenderBundle>();
  ...
  private tileLoader?: MvtTileLoader;
  private readonly tileLoaderFactory?: (styleSet: MvtStyleSet) => MvtTileLoader;
  private readonly tileStore: MvtTileStore;
  private readonly tilingScheme: TilingScheme;
  private styleSet?: MvtStyleSet;

  constructor(options: MvtTilesetPrimitiveOptions) {
    ...
    this.sourceTileCache = new MvtSourceTileCache({
    ...
    this.parseWorkerClient = new MvtParseWorkerClient({
      debugLogging: this.debugLogging,
    });
    this.warningContext = createMvtWarningContext(this.debugLogging);
  }
  ...
  getStats(): MvtTilesetPrimitiveStats {
    return {
  ...
  setStyleSet(styleSet: MvtStyleSet): void {
  ...
  update(frameState: MvtPrimitiveFrameState): void {
  ...
  private flushParseQueue(styleSet: MvtStyleSet): void {
    let parsedBytes = 0;
    ...
    while (parsedTileCount < this.maxParseTilesPerFrame) {
      ...
      if (!this.parseWorkerClient.isWorkerEnabled()) {
        try {
          const parsedTileData = parseMvtVectorTile(
  ...
  private failTile(tile: MvtTile, stage: string, error?: unknown): void {
    logMvtError(this.debugLogging, `Tile ${stage} 失败。`, {
      coordinate: tile.coordinate,
      sourceCoordinate: tile.sourceCoordinate,
  ...
  private startPendingLoads(tileLoader: MvtTileLoader): void {
  ...
  private async startTileLoad(tile: MvtTile, tileLoader: MvtTileLoader): Promise<void> {
  ...
  private resolveSourceCoordinate(tile: MvtTile): MvtTile['sourceCoordinate'] {
    return getMvtSourceTileCoordinate(tile.coordinate, this.styleSet?.source?.maxzoom);
  }
}

function createDefaultTileLoader(styleSet: MvtStyleSet): MvtTileLoader {
  const tileTemplate = styleSet.getSourceTileTemplate();
  if (!tileTemplate) {
    throw new Error('MVT vector source does not define a tile template.');
  }

  return new DefaultMvtTileLoader({
    tileTemplate,
  });
}
