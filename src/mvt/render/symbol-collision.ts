...
export class SymbolCollisionIndex {
  private readonly grid: GridIndex<CollisionKey>;
  private readonly ignoredGrid: GridIndex<CollisionKey>;
  ...
  constructor() {
    this.grid = new GridIndex(1, 1, CELL_SIZE);
    this.ignoredGrid = new GridIndex(1, 1, CELL_SIZE);
  }
  ...
  collides(box: SymbolCollisionBox): boolean {
    if (this.scene) {
  ...
  insert(box: SymbolCollisionBox): void {
    if (this.scene) {
  ...
  private collidesTile(box: SymbolCollisionBox): boolean {
  ...
  private insertTile(box: SymbolCollisionBox): void {
    this.tileBoxes.push({
      maxX: box.tileMaxX,
      maxY: box.tileMaxY,
      minX: box.tileMinX,
  ...
  private collidesScreen(box: SymbolCollisionBox): boolean {
    const screenPosition = SceneTransforms.worldToWindowCoordinates(
      this.scene!,
      box.position,
    );
  ...
  private insertScreen(box: SymbolCollisionBox): void {
    const screenPosition = SceneTransforms.worldToWindowCoordinates(
      this.scene!,
      box.position,
    );
