...
export function buildMvtStyleFamilies(
  styleLayers: readonly MvtStyleLayerSpecification[],
  compiledLayerMap: ReadonlyMap<string, MvtCompiledStyleLayer>,
  sourceId?: string,
): MvtStyleFamily[] {
  const families: MvtStyleFamily[] = [];
  for (const layoutGroup of groupByLayout([...styleLayers] as LayerSpecification[])) {
    const compiledLayers = layoutGroup
      ...
      .filter((compiledLayer): compiledLayer is MvtCompiledStyleLayer => Boolean(compiledLayer));
    ...
    families.push({
      key: createMvtStyleFamilyKey(firstLayer),
...
export function compileStyleLayer(
  styleLayer: MvtStyleLayerSpecification,
  order: number,
  ...
  return {
    filterEvaluator: createMvtFilterEvaluator(filterSpecification),
    filterKey,
    filterSpecification,
    id: styleLayer.id,
    layout,
...
export function createMvtStyleFamilyKey(styleLayer: MvtCompiledStyleLayer): string {
  return [
    styleLayer.source ?? '',
