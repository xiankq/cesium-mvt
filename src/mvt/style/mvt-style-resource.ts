...
export async function fetchTileJson(tileJsonUrl: string, fetcher?: typeof fetch): Promise<MvtTileJson> {
  return await fetchStyleJsonResource<MvtTileJson>(
    tileJsonUrl,
    fetcher,
...
export async function resolveStyleSpriteAtlas(
  ...
  fetcher?: typeof fetch,
): Promise<MvtStyleSpriteAtlas | undefined> {
  if (!spriteUrl) {
    return undefined;
  }
  ...
  const [spriteIndex, spriteImageBuffer] = await Promise.all([
    fetchStyleJsonResource<Record<string, MvtStyleSpriteEntry>>(
      jsonUrl,
      fetcher,
...
function readPngSize(arrayBuffer: ArrayBuffer): MvtPngSize {
