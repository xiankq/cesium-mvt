...
import type { BucketFeature, CircleMeshBucket, FillMeshBucket, LineMeshBucket, MeshBucket, MeshIndexArray, ParsedTileData, PointBucketFeature, TileBucket, TileMeshData } from '../types';
import { BoundingRectangle, Cartesian2, IndexDatatype } from '@cesium/engine';
import earcut from 'earcut';
import { normalizePolylinePoints, normalizeRingPoints } from './geometry-normalize';
...
export function buildTileMesh(parsedTileData: ParsedTileData): TileMeshData {
...
function appendLineFeature(
  feature: Extract<BucketFeature, { geometryType: 'LineString' }>,
  positions: number[],
  extrudes: number[],
  indices: number[],
  boundsPoints: Cartesian2[],
...
function appendPolygonFeature(
  feature: Extract<BucketFeature, { geometryType: 'Polygon' }>,
  positions: number[],
  indices: number[],
  boundsPoints: Cartesian2[],
): void {
...
function buildBucketMesh(bucket: TileBucket): MeshBucket | undefined {
  switch (bucket.type) {
    case 'fill':
...
function buildCircleMeshBucket(bucket: TileBucket): CircleMeshBucket {
...
function buildFillMeshBucket(bucket: TileBucket): FillMeshBucket {
...
function buildLineMeshBucket(bucket: TileBucket): LineMeshBucket {
  const positions: number[] = [];
...
function createIndexArray(vertexCount: number, indices: number[]): MeshIndexArray {
