...
function compareQueuedTileJobs(a: QueuedTileJob, b: QueuedTileJob): number {
...
export class TileScheduler {
  ...
  constructor(
    ...
    const resolvedMaxConcurrentRequests = Math.max(
      2,
      Math.min(8, Math.floor(maxConcurrentRequests ?? resolveHardwareConcurrency())),
    );
